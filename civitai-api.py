...
def register_checkpoint(checkpoint_path):
    """
    Register a freshly downloaded checkpoint with the WebUI and load it immediately.
    """
    try:
        ...
        return f"Loaded checkpoint '{info.title}'"
    except Exception as exc:  # pragma: no cover - defensive
        return f"Failed to load checkpoint: {exc}"


def download_file_thread(url, file_name, content_type, use_new_folder, model_name, lora_old, auto_load_checkpoint=False):
    ...
    def runner():
        try:
            download_file(url, path_to_new_file)
            if auto_load_checkpoint and content_type == "Checkpoint":
                status = register_checkpoint(path_to_new_file)
                print(status)
    ...
    thread.start()
    msg = f"Downloading to {path_to_new_file}"
    if auto_load_checkpoint and content_type == "Checkpoint":
        msg += " and will load when complete"
    return msg


def download_file_and_load(url, file_name, content_type, use_new_folder, model_name, lora_old):
    return download_file_thread(url, file_name, content_type, use_new_folder, model_name, lora_old, auto_load_checkpoint=True)

def load_selected_model(model_filename, content_type, use_new_folder, model_name, lora_old):
    """
    Load an already-downloaded checkpoint directly into WebUI.
    """
    if not model_filename or model_filename == PLACEHOLDER:
        return "Select a model file first."
    if content_type != "Checkpoint":
        return "Auto-load is only supported for Checkpoint content types."

    model_folder = make_new_folder(content_type, use_new_folder, model_name, lora_old)
    checkpoint_path = os.path.join(model_folder, model_filename)
    if not os.path.exists(checkpoint_path):
        return f"File not found. Expected at: {checkpoint_path}"

    return register_checkpoint(checkpoint_path)
...
def on_ui_tabs():
    with gr.Blocks() as civitai_interface:
        ...
        with gr.Row():
            ...
            status = gr.Textbox(label="Civitai Status", interactive=False, value=None)
        with gr.Row():
            ...
            download_and_load = gr.Button(value="5th - Download && Load (Checkpoint)")
            load_local = gr.Button(value="Load Selected (Checkpoint)")
            with gr.Row():
        ...
        download_and_load.click(
            fn=download_file_and_load,
            inputs=[
            dl_url,
            model_filename,
            content_type,
