...
def initialize():
    ...
    # If we ended up on CPU (e.g., unsupported CUDA arch), force float32 to avoid half/float mix errors.
    if devices.device.type == "cpu":
        # Force runtime to stay in float32 everywhere.
        cmd_opts.no_half = True
        cmd_opts.no_half_vae = True
        cmd_opts.precision = "full"
        devices.dtype = devices.dtype_vae = devices.dtype_inference = torch.float32
        devices.force_fp16 = False
