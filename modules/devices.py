...
def _cuda_arch_supported() -> bool:
    """
    Verify the current CUDA device has kernels compiled in this torch build.
    """
...
def cuda_no_autocast(device_id=None) -> bool:
    # Avoid touching CUDA APIs when CUDA is not compiled/available.
    if not torch.cuda.is_available():
        return False
...
def get_optimal_device_name():
    # Prefer CUDA if compiled, available, and the device architecture is supported by this torch build.
    if torch.cuda.is_available():
        if _cuda_arch_supported():
            return get_cuda_device_string()
        else:
            try:
                major, minor = torch.cuda.get_device_capability()
                print(f"Warning: CUDA device with capability sm_{major}{minor} not supported by this torch build; falling back to CPU.")
            except Exception:
                print("Warning: CUDA device not supported by this torch build; falling back to CPU.")
